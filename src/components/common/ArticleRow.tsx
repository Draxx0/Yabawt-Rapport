...
const ArticleRowText = ({ children, articleSubtitle, hasButton = false, textButton = "Découvrir", url }: { children: React.ReactElement, articleSubtitle: string; hasButton?: boolean, textButton?: string; url?: string; }) => (
  <div className="flex flex-col gap-6 w-full md:w-1/2">
    <h2 className="text-2xl w-fit font-bold relative z-10 before:[] before:w-1/2 before:h-2 before:absolute before:left-0 before:bottom-[0.15rem] -before:z-10 before:bg-red before:opacity-40 before:rounded-sm">{articleSubtitle}</h2>
    {children}
